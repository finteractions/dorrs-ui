...
class ReportsService extends BaseService {
    ...
    public async getSummary(data: any): Promise<Array<IReportLastSaleTotalByAlternativeTradingSystem | IReportLastSaleTotalForEachSymbol | IReportNumberOfSymbolAdditionsAndDeletions>> {
        // return (await apiWebBackendService.post<IResponse<Array<IReportLastSaleTotalByAlternativeTradingSystem | IReportLastSaleTotalForEachSymbol | IReportNumberOfSymbolAdditionsAndDeletions>>>(`${this.PATH}generate/`, data,  {}, this.getUserAccessToken())).data;
        ...
        if (data.report === ReportType.LAST_SALE_TOTALS_BY_ATS) {
            res = d1;
        } else if (data.report === ReportType.LAST_SALE_FOR_EACH_SYMBOL) {
            res = d2;
