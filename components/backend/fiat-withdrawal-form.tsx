...
interface FiatWithdrawalFormState extends IState {
    mode: string;
    isConfirmedApproving: boolean;
    loading: boolean;
    isApproving: boolean | null;
    data: ICustody | null;
    bank_account: IAdminBankAccount | null;
}

interface FiatWithdrawalFormProps extends ICallback {
    action: string;
    data: ICustody | null;
    onCancel?: () => void;
...
class FiatWithdrawalForm extends React.Component<FiatWithdrawalFormProps, FiatWithdrawalFormState> {
    commentTextarea = React.createRef<HTMLTextAreaElement>();
    state: FiatWithdrawalFormState;

    constructor(props: FiatWithdrawalFormProps) {
        super(props);
    ...
    componentDidMount() {
        if (this.state.data?.bank_account) {
            ...
            this.getBankAccount(this.state.data?.bank_account || 0);
        }
    }

    getBankAccount = (id: number) => {
        adminService.getBankAccount(id)
            .then((res: IAdminBankAccount) => {
                this.setState({bank_account: res})
            })
            .catch((errors: IError) => {
    ...
    updateFiatWithdrawal = () => {
        adminService.getFiatWithdrawal(this.state.data?.id || 0)
            .then((res: ICustody) => {
                this.setState({data: res});
                this.getBankAccount(res.bank_account || 0)
            })
            .catch((errors: IError) => {
                this.setState({errorMessages: errors.messages, loading: false, isApproving: null, isConfirmedApproving: false})
            });
                ...
                return (
                    ...
                    {this.state.loading ? (
                        ...
                        <>
                            ...
                            <div className='view-form fiat-withdrawal-view-form'>
                                ...
                                <div className="view-form-box">
                                    ...
                                    <div className="box__wrap">{formatterService.numberFormat(this.state.data?.base_price)} {this.state.data?.base_currency}</div>
                                </div>
                                <div className="view-form-box">
                                    <div className="box__title">Bank Account</div>
                                    <div className="box__wrap">{this.state.bank_account ? `${this.state.bank_account.account_number} (${this.state.bank_account.beneficiary_name})` : ''}</div>
                                </div>
                                <div className="view-form-box">
                                ...
                                <div className="view-form-box">
                                    ...
                                    <div className="box__wrap">{formatterService.dateTimeFormat(this.state.data?.date_time || '')}</div>
                                </div>
                            </div>
                        </>
                    )}
