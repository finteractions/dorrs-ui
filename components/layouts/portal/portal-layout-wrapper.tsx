...
function PortalLayoutWrapper({children}: PortalLayoutProps) {
    ...
    useEffect(() => {
        if (localStorage.getItem('isPortalShowSidebarMd')) {
            setIsShowSidebarMd(localStorage.getItem('isPortalShowSidebarMd') === 'true')
        }
    }, [setIsShowSidebarMd])
    return (
        <div className="portal">
            <div ref={ref} className="position-absolute w-100"/>
            <PortalSidebar isShow={isShowSidebar} isShowMd={isShowSidebarMd} toggleIsShowSidebarMd={toggleIsShowSidebarMd}/>
